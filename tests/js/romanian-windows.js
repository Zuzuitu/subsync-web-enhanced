...
assert.strictEqual(WINDOW_SECONDS, 30);
assert.strictEqual(MAX_WINDOWS, 16);
assert.strictEqual(FULL_SCAN_SECONDS, 480);
assert.strictEqual(makeRomanianTimeWindows(undefined), null);
assert.strictEqual(makeRomanianTimeWindows(480), null);

const duration = 7200;
const windows = makeRomanianTimeWindows(duration);
assert.strictEqual(windows.length, 16);
...
assert.strictEqual(
  ...
  FULL_SCAN_SECONDS,
  'a long movie should cap Romanian ASR input at eight minutes'
);
