...
assert.strictEqual(MIN_INFORMATIVE_WORDS, 8);
...
let state = tracker.observe(
  { start: 0, end: 15 },
  20,
  { correlated: true, formula: { a: 1, b: -4 } }
);
...
state = tracker.observe(
  { start: 7185, end: 7200 },
  0,
  { correlated: true, formula: { a: 1, b: -7.5 } }
);
assert.strictEqual(state.verified, false);
assert.strictEqual(
...
state = tracker.observe(
  { start: 3500, end: 3515 },
  25,
  { correlated: true, formula: { a: 1, b: -8.0 } }
);
assert.strictEqual(state.stableCorrelatedWindows, 1);
assert.strictEqual(state.verified, false);
...
state = tracker.observe(
  { start: 1700, end: 1715 },
  18,
  { correlated: true, formula: { a: 1.00002, b: -8.05 } }
);
...
state = tracker.observe(
  { start: 5300, end: 5315 },
  22,
  { correlated: true, formula: { a: 1.00001, b: -8.10 } }
);
...
for (let i = 0; i < 5; i++) {
  state = sparse.observe(
    ...
    20,
    { correlated: true, formula: { a: 1, b: -8 } }
  );
}
assert.strictEqual(
  state.verified,
